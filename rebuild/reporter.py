"""
rebuild.reporter — generuje raport HTML + JSON dla wyników dnia.
...
def save_json(result: DayResult) -> None:
...
def _status_badge(status: EndpointStatus) -> str:
    colors = {
...
def _screenshot_html(r: EndpointResult, day_dir: Path) -> str:
...
def save_html(result: DayResult) -> None:
    out = result.output_dir
    if not out:
        return
    out.mkdir(parents=True, exist_ok=True)
    ...
    for r in result.endpoint_results:
        ...
        screenshot = _screenshot_html(r, out)
        rows += f"""
        <tr>
...
def save_day(result: DayResult) -> None:
    """Zapisuje pełny raport dnia: JSON + HTML."""
    save_json(result)
    save_html(result)


# ──────────────────────────────────────────────
# Timeline index (po wszystkich dniach)
# ──────────────────────────────────────────────

def save_timeline_index(results: list[DayResult], output_dir: Path) -> None:
    """Generuje .rebuild/index.html z timelineą wszystkich dni."""
